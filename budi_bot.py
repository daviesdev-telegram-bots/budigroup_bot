...
def all_messages(message: Message):
    if get_user(message.chat.id).is_disabled:
        bot.send_message(message.chat.id, "Your account has been suspended.\n\nPlease contact support for more info", reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("Contact", support)))
...
def callback_query_handler(callback: CallbackQuery):
    ...
    if data == "register":
        ...
        if number_data.get("activationId"):
            ...
            bot.edit_message_text(number_data["message"], message.chat.id, message.id, reply_markup=InlineKeyboardMarkup().add(back_btn("order_service")))
